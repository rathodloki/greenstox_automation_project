...
def channel_invite_button(update, context):
            user_id = update.effective_user.id
            chat_info = bot.get_chat(chat_id=user_id)
            username = chat_info.username
            ...
            text = "*⏳ Expired in 12hrs! Join our channel now.*" 
            context.bot.send_message(chat_id=update.effective_chat.id, text=text, parse_mode=telegram.ParseMode.MARKDOWN ,reply_markup=reply_markup)
    ...
    try:
        ...
        if code in codes:
            if code[0] == "0": # If code starts with 0, it's a 1 day license
                ...
                d = 90
            elif code[0] == "4": # If code starts with 4, it's a 180 days license
                d = 180
            elif code[0] == "5": # If code starts with 5, it's a 365 days license
                d = 365
            else:
                # handle invalid code
            ...
            chat_info = bot.get_chat(chat_id=user_id)
            username = chat_info.username
            fullname = chat_info.full_name
            
            # Send confirmation message to user
            bot.send_message(chat_id=user_id, 
                 text=f"<b>✅ License redeemed!</b>\n\nHi <a href='tg://user?id={user_id}'>{fullname}</a>\nAdded {d} days access.\n\n<b>Expire</b>: {removal_date}", 
                 parse_mode="HTML")
            if(user_exist):
                channel_invite_button(update, context)

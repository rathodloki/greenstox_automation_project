...
export default async function (payload, env) {
  ...
  const { channel_post} = payload
  if (message || channel_post) {
    if (message) { 
      var { chat, text, from } = message;
   } 
   else { 
     var { chat, text, from } = channel_post;
    }
    if (chat.id !== 635834411 && chat.id !== -1002011835652 && chat.id !== 6040970776) {
       await procCommand(chat, "/not_allowed" , env)
    }
    else if (text && text.startsWith('/')) {
      chat.id = -1002011835652
      ...
      if (chat.type === ChatType.PRIVATE) {
        ...
        if (isPrivateMessageEnabled()) {
            log.info(`${chat.type} message from ${chat.username} (${chat.id}) - ${cmdText}`) // prettier-ignore
            await procCommand(chat, cmdText, env)
        } else {
          log.verbose('Channel message is disabled')
        }
      }
